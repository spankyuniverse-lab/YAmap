...
# Последовательность ответов после выбора режима «1»:
# покрытие → что собираем → браузеры → имя файла → подтверждение.
CASES = [
    # Все дефолты: 19 городов, весь GT, 2 браузера
    ...
    ("1\n\n2\n2\n1\n\n", ["--all-cities", "--category", "gt-fuel", "-o", "1.xlsx",
                          "--workers", "2", "--api-intercept"]),
    # Аулы + сельский набор → свой файл kz_aul.xlsx и --cities аулы
    ("1\n3\n5\n2\n\n\n", ["--all-cities", "--category", "gt-село", "-o", "kz_aul.xlsx",
                          "--workers", "2", "--api-intercept", "--cities", "аулы"]),
    # МАКСИМУМ (города + аулы) + весь GT → kz_max.xlsx и --cities макс
    ("1\n4\n1\n2\n\n\n", ["--all-cities", "--category", "gt", "-o", "kz_max.xlsx",
                          "--workers", "2", "--api-intercept", "--cities", "макс"]),
]
fails = []
