"""CSS-селекторы Я.Карт + автодетект через JS.

Яндекс.Карты периодически меняют DOM и обфусцируют классы.
Цепочка резолва: кеш → hardcoded → auto-detect.
"""
...
# ---------------------------------------------------------------------------
# Hardcoded селекторы (для fallback)
# Используем [class*="..."] — поиск по подстроке класса, чтобы переживать
# числовые префиксы вроде "_1a2b3c_search-snippet-view".
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# JS-эвристики для автодетекта (выполняются через page.evaluate())
# ---------------------------------------------------------------------------
...
_JS_DETECT_SNIPPETS = """() => {
    ...
    let best = null;
    for (const base in candidates) {
        const info = candidates[base];
...
def detect_selectors(page: Page, mode: str = "list") -> dict[str, str]:
    ...
    Args:
        mode: 'list' — селекторы списка/сниппетов;
              'detail' — селекторы карточки организации.
    """
    found: dict[str, str] = {}
...
# ---------------------------------------------------------------------------
# SelectorEngine — цепочка резолва: cache → hardcoded → auto-detect
# ---------------------------------------------------------------------------
