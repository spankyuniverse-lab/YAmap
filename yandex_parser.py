...
def _point_in_kz(lon: float, lat: float) -> bool:
    """Точка внутри полигона границы РК (ray casting). Без полигона — True."""
    poly = KZ_BORDER
    n = len(poly)
    if n < 3:
        return True
...
def _tile_in_kz(lon: float, lat: float, span: float) -> bool:
    """Тайл (центр + span) задевает территорию РК? Центр или любой угол внутри.
...
def country_grid(step_deg: float = COUNTRY_STEP_DEG,
                 bbox: tuple[float, float, float, float] = KZ_BBOX,
                 clip: bool = True) -> list[str]:
    """Сетка вьюпортов по территории страны (для сплошного сбора).

    clip=True (по умолчанию) выбрасывает тайлы вне полигона границы РК:
    Каспий, РФ, Узбекистан, Киргизия, Китай — это ~40% прямоугольника bbox.
    """
    ...
    while lat <= lat_max + 1e-9:
        ...
        while lon <= lon_max + 1e-9:
            if not clip or _tile_in_kz(lon, lat, step):
                tiles.append(f"{lon:.4f},{lat:.4f}")
            lon += step
        lat += step
    return tiles
...
def _grid_snap(lon: float, lat: float, step: float) -> tuple[int, int]:
    """Индекс ячейки национальной сетки для точки (та же решётка, что
    country_grid: начало отсчёта — юго-западный угол KZ_BBOX)."""
    lon_min, lat_min, _, _ = KZ_BBOX
...
def set_viewport(city: str | None = None, ll: str | None = None, z: int | None = None) -> None:
    ...
    elif city and city in KZ_PLACES_ALL:
        MAP_LL, MAP_Z = KZ_PLACES_ALL[city], (z or DEFAULT_CITY_Z)
    else:
            ...
            while queue and processed < MAX_UNITS:
                ...
                if saturated:
                    ...
                    for slon, slat in _subtile_centers(lon, lat, span):
                        ...
                        if not _tile_in_kz(slon, slat, sub_span):
                            continue
